...
'  This function scans version info in both assembly identity and dependency elements
...
Function ChkAllGuidtypes(byRef rootdocobj, byRef strerr)
	...
	numberofarrAttribsToChk = 3  'because VBScript doesnt have a way to dynamically get the size of an array you must keep track yourself
...
Function IsValidCommandLine()
    ...
    ' nOnlyAllowFirstTimeReadFlag values: Manifest = 1 Schema = 2 Quiet = 4 InBuildProcess = 8 ManifestType = 16
...
' Verifies by confirming that the XML3 .load method does not report an error
...
' Use chkType to specify whether Publisher or Application configuration
...
Function ChkCFGversion(byRef rootdocobj, byRef strerr, chkType)
    ...
    Dim versionregexp
    versionregexp = "^[0-9]{1,5}\.[0-9]{1,5}\.[0-9]{1,5}\.[0-9]{1,5}$"
    Dim versionregexprange
    versionregexprange = "^[0-9]{1,5}\.[0-9]{1,5}\.[0-9]{1,5}\.[0-9]{1,5}-[0-9]{1,5}\.[0-9]{1,5}\.[0-9]{1,5}\.[0-9]{1,5}$"
    ...
    'Now loop through each array string item and do the search and validation check
    'note: the array is zero based
    For i = 0 To 1 Step 1
	    'First check version at the /assembly/assemblyIdentity level
	    set versionChkList = rootdocobj.selectNodes(arrayofelementstosearch(i))
	       ...
	       For counter = 0 To versionChkList.length-1 Step 1
	           MyVar = UCase (CStr(versionChkList.item(counter).text))
		           if RegExpTest(versionregexp, MyVar) then
		              ...
		              if i = 0 then
		              ' Single oldVersion format failed.  See if version range format is used.
				' Assumes both Policy and Application Config files accommodate oldVersion range format.
					if RegExpTest(versionregexprange, MyVar) then
				  	    retVal = True
				  	else	              
...
'gRunMode - flag that identifies which user flags have been asserted.
' value  1 - bit indicates run quiet /Q
' value  2 - bit indicates in build process /B
' value  8 - bit indicates /AM manifest type
' value 16 - bit indicates /PC manifest type
' value 32 - bit indicates /AC manifest type
Dim gRunMode 
