...
#pragma pack(pop)
	...
	// For AMD64 we will stick the this pointer into RCX, which is where
	// the HWND is.  However, we don't actually need the HWND so this is OK.
#pragma pack(push,1)
struct _WndProcThunk
{
	BYTE				m_rex;		// specify 64-bit operand size
	BYTE				m_mov;		// mov RCX, pThis (RCX is hWnd)
	DWORD_PTR			m_this;		//
	BYTE				m_jmp;		// jmp WndProc
...
class CDynamicWndProcThunk
{
public:
	_WndProcThunk *pThunk;

	CDynamicWndProcThunk()
	{
		pThunk = NULL;
	}
	...
	{
		ATLASSERT(!pThunk);
		pThunk = static_cast<_WndProcThunk *>(HeapAlloc(GetProcessHeap(), 
			HEAP_GENERATE_EXCEPTIONS, sizeof(_WndProcThunk)));
		ATLASSERT(pThunk);
	}
};

#if defined (_M_AMD64)
typedef CDynamicWndProcThunk CDynamicThunk;
#else
typedef _WndProcThunk CDynamicThunk;
#endif
...
public:
   ...
   thunk.pThis = pThis;
#elif defined (_M_AMD64)
		thunk.pThunk->m_rex = 0x48;
		thunk.pThunk->m_mov = 0xb9;
		thunk.pThunk->m_this = (DWORD_PTR)pThis;
		thunk.pThunk->m_jmp = 0xe9;
		thunk.pThunk->m_relproc = (DWORD_PTR)((__int64)proc - (__int64)thunk.pThunk + sizeof(_WndProcThunk));
#endif
		// write block from data cache and
		//  flush from instruction cache
#if defined (_M_AMD64)
		FlushInstructionCache(GetCurrentProcess(), thunk.pThunk, sizeof(_WndProcThunk));
#else
