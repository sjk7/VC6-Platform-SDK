...
LRESULT CToolBar::OnSetSizeHelper(CSize& size, LPARAM lParam)
{
	...
	if (lResult)
      //IA64: Assume size is packed in low-order DWORD
		size = DWORD(lParam);
...
void CToolBar::Dump(CDumpContext& dc) const
{
	...
	dc << "m_hbmImageWell = " << (UINT_PTR)m_hbmImageWell;
	dc << "\nm_hInstImageWell = " << (UINT_PTR)m_hInstImageWell;
	dc << "\nm_hRsrcImageWell = " << (UINT_PTR)m_hRsrcImageWell;
