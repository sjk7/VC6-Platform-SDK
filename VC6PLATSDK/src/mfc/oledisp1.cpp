	...
	VARIANT* pvarResult, DISPPARAMS* pDispParams, UINT* puArgErr)
{
	...
	if(!ATL::_ATL_SAFE_ALLOCA_IMPL::_AtlVerifyStackAvailable(nSizeArgs + _SCRATCH_SIZE + _ALIGN_STACKPOINTER))
			return E_OUTOFMEMORY;
...
#endif
#ifdef _AMD64_
	memset(pStack, 0, nSizeArgs+_SCRATCH_SIZE);  // Zero out stack
#endif
	...
	if (sc == S_OK)
	{
		TRY
		{
			...
			case VT_DATE:
				...
				break;
			case VT_CY:
				result.cyVal = ((CY (AFXAPI*)(AFX_PMSG, void*, UINT))
					pfnDispatch)(pfn, pStack, nSizeArgs);
				break;
			case VT_VARIANT:
				result.vaVal = ((VARIANT (AFXAPI*)(AFX_PMSG, void*, UINT))
					pfnDispatch)(pfn, pStack, nSizeArgs);
				break;
