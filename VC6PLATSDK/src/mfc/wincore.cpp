...
public :
#ifdef _AFX_DLL_IMPL
	__declspec(noinline) virtual ~CTempWnd() { }	
#endif
};
...
BOOL AFXAPI AfxRegisterClass(WNDCLASS* lpWndClass)
{
	...
	if (afxContextIsDLL)
	{
		...
		TRY
		{
			...
			//
			// In retail, fail if the above assert would've failed.
			if (lstrlen(lpszUnregisterList) + lstrlen(lpWndClass->lpszClassName) + 2 >=
				_countof(pModuleState->m_szUnregisterList)) 
			{
					return FALSE;
			}
