...
#else
		if (::LoadString(hRes, nResource, szBuffer, sizeof(szBuffer)/sizeof(szBuffer[0])) > 0)
#endif
		{
			pCtxt->Reset();
			CHttpServer::StartContent(pCtxt);

			if (::LoadString(hRes, AFX_IDS_HTTP_TITLE, szTitle, sizeof(szTitle)/sizeof(szTitle[0])) > 0)
			{
	...
	AFX_PARSEMAP_ENTRY* pParams, LPTSTR pszParams)
{
	...
	if (nRet == callOK)
	{
		...
		{
            ATL::_ATL_SAFE_ALLOCA_IMPL::_Atlresetstkoflw();
		}
